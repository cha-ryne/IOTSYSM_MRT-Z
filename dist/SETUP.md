...
This installs the `gh-pages` package used for deployment.

### 5.3 Build the static site

```bash
npm run build
```

This creates a `dist/` folder containing the files that GitHub Pages will host.

### 5.4 Publish to the `gh-pages` branch
...
### 5.6 Automatic deployment
The repository includes [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml), which runs on every push to `main` and publishes the site automatically.
